...
def main():
    ...
    parser = argparse.ArgumentParser(description="TMDS scale demo on Forest Covertype.")
    ...
    X, y = fetch_covtype(return_X_y=True, shuffle=True, random_state=0)
    if args.max_rows:
        X, y = X[: args.max_rows], y[: args.max_rows]
    ...
    raw_clusters = MiniBatchKMeans(n_clusters=T.shape[1], n_init=5, random_state=0).fit_predict(Y_test)
    raw_cluster_seconds = time.perf_counter() - score_start
    cluster_start = time.perf_counter()
    tmds_clusters = MiniBatchKMeans(n_clusters=T.shape[1], n_init=5, random_state=0).fit_predict(Z_test)
    tmds_cluster_seconds = time.perf_counter() - cluster_start
    tmds_class_pred = (t_mean + Z_test).argmax(axis=1)
    majority_class = np.bincount(y_train).argmax()
    kmeans_seconds = raw_cluster_seconds + tmds_cluster_seconds
    ...
    dense_arrays_mib = shared_mib + tmds_specific_mib
